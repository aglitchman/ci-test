...
- [x] Build Bob.jar for Windows x64.
- [x] Modify Bob.jar to optionally output shaders in format GLES_SM300 and GLES_SM100 for Windows.
    - [ ] Propose a new field to `ext.manifest` like `shaderVersion` or `shaderOutput`.
    - [ ] Add Bob.jar customizations to Defold repo.
- [ ] Add ANGLE args like `--use-angle=d3d9` to the extension.
- [ ] Build statically linked ANGLE, because the license is permissive, BSD-3.
